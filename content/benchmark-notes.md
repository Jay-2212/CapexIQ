...
- **Low** — a weak source: a broad market claim without clear methodology, an
  ...
  one indirect source.
- **Unavailable** — no responsible source was found at all. The field is left blank
  (`null`) rather than filled with a guess. You'll need to enter your own figure.
...
plausible-looking guess — target IRR/hurdle rate is one example (no Indian
hospital/investor benchmark for this is publicly disclosed anywhere, even after
multiple dedicated research passes). This is a deliberate choice, not a gap someone
forgot to fill: a field with no evidence behind it is more honest left blank, with a
clear "no benchmark available, enter your own estimate" note, than papered over with an
invented number that merely looks authoritative. See `ISSUES.md` for the running list
of what's currently unresearched and why.
